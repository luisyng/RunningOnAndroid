 ...
 *
 */
public class BalloonOverlayView<CompleteItem> extends FrameLayout {
	...
	public BalloonOverlayView(Context context, int balloonBottomOffset) {
		...
		vista = (ImageView) v.findViewById(R.id.ballon_item_photo);
	 ...
	 * (title and snippet). 
	 */
	public void setData(running.android.app.CommentOverlayItem item) {
		...
		if (item.getPhoto() != null){
			imagen = BitmapFactory.decodeByteArray(item.getPhoto(), 0, 
					item.getPhoto().length);
			vista.setImageBitmap(imagen);
		} else {
			vista.setVisibility(GONE);
		}	
	}
}
